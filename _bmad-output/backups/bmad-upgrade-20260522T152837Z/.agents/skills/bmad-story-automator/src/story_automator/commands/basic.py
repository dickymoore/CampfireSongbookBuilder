...
from ..core.story_keys import normalize_story_key, sprint_status_file
...
def cmd_commit_story(args: list[str]) -> int:
    ...
    staged_paths = _commit_story_paths(Path(repo), story)
    if not staged_paths:
        write_json({"ok": False, "error": "no_story_paths"})
        return 1

    scoped_status = run_cmd(
        "git",
        "-C",
        repo,
        "status",
    ...
    scoped_lines = [line for line in scoped_status.output.strip().splitlines() if line.strip()]
    if not scoped_lines:
        write_json({"ok": False, "error": "no_story_changes", "paths": staged_paths})
        return 0

    if run_cmd("git", "-C", repo, "add", "-A", "--", *staged_paths).exit_code != 0:
        write_json({"ok": False, "error": "git_add_failed", "paths": staged_paths})
        return 1
    message = f"feat(story-{story}): {title}"
    commit = run_cmd("git", "-C", repo, "commit", "-m", message)
    if commit.exit_code != 0:
        write_json({"ok": False, "error": "commit_failed", "paths": staged_paths})
        return 1
    sha = run_cmd("git", "-C", repo, "rev-parse", "HEAD").output.strip()
    write_json({"ok": True, "commit": sha, "paths": staged_paths})
    return 0


def _commit_story_paths(repo_path: Path, story: str) -> list[str]:
    norm = normalize_story_key(str(repo_path), story)
    if norm is None:
        return []

    story_file = _find_story_file(repo_path, norm.prefix, norm.key)
    if story_file is None:
        return []

    paths = [story_file]
    sprint_file = Path(sprint_status_file(str(repo_path)))
    if sprint_file.exists():
        paths.append(sprint_file)

    for relative_path in _extract_story_file_list(story_file):
        candidate = repo_path / relative_path
        if candidate.exists() or _git_path_tracked(repo_path, relative_path):
...
def _find_story_file(repo_path: Path, prefix: str, key: str) -> Path | None:
    artifacts = repo_path / "_bmad-output" / "implementation-artifacts"
    exact = artifacts / f"{key}.md"
    if exact.exists():
        return exact

    matches = sorted(
        artifacts.glob(f"{prefix}-*.md"),
...
def _extract_story_file_list(story_file: Path) -> list[str]:
...
def _git_path_tracked(repo_path: Path, relative_path: str) -> bool:
    result = run_cmd(
        "git",
        "-C",
        str(repo_path),
