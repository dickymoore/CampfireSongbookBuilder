...
def _story_file_status(args: list[str]) -> int:
    ...
    if norm is None:
        ...
        return 1
    artifacts = Path(get_project_root()) / "_bmad-output" / "implementation-artifacts"
    exact = artifacts / f"{norm.key}.md"
    if exact.exists():
        target = exact
    else:
        matches = sorted(
            artifacts.glob(f"{norm.prefix}-*.md"),
    ...
    if not target.exists():
        print_json({"ok": False, "error": "story file not found", "prefix": norm.prefix})
        return 1
    print_json({"ok": True, "story_key": norm.key, "file": str(target), "status": find_frontmatter_value_case(target, "Status") or "unknown", "title": find_frontmatter_value_case(target, "Title")})
    return 0
