...
- ANALYZE ONLY THE ACTIVE SOURCES NEEDED TO WRITE THE STORY: prefer the current sprint status entry, current epics file, current architecture file, the newest relevant PRD/addendum, project-context facts, and the immediately previous story when helpful
- AUTONOMOUS RUNS MUST BE WRITE-FIRST: create the target story file from the template as soon as the story key, title, and scope are resolved, then fill and refine it in place
- DO NOT DRIFT INTO OPEN-ENDED RESEARCH: if the required local planning artifacts are present, do not browse the web and do not spawn subprocesses or subagents for extra exploration
...
- COMPLETION CONTRACT: the workflow is only complete when the canonical story file exists at the target output path and sprint status has moved from `backlog` to `ready-for-dev`
