...
When this protocol runs inside an autonomous create-story worker, optimize for deterministic completion instead of broad research:

- Prefer the active local planning artifacts already selected by the current roadmap.
- Load the minimum set needed to write the target story accurately.
- Do not browse the web if the needed local files are present.
- Do not keep loading alternate or historical documents after a current authoritative source is found.
- Once the story scope is clear, return control so the worker can create the story file immediately.
...
**Autonomous create-story note:** if a current whole-document source already exists for this input and clearly covers the active story, prefer that current whole document over broad shard traversal.
...
**Autonomous create-story exception:** when multiple historical versions exist, choose the newest roadmap-relevant file and stop. Do not load older sibling versions unless the active file explicitly points to them.
...
**Autonomous create-story refinement:** if multiple whole-document matches exist, sort by recency and roadmap relevance, choose the best current match first, and only load additional matches if the selected file is missing required story details.
